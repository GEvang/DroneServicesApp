...
class Survey(private var area : PolygonArea, var activity: FragmentActivity)
{
    ...
    fun createSurveyPath(distance: Double, angle : Int, context: Context?, map: GoogleMap): ArrayList<LatLng>
    {
        ...
        {
            Toast.makeText(context, context?.getString(R.string.area_too_big_msg), Toast.LENGTH_LONG).show()
            return result
        }
    ...
    private fun angledSurvey(distance: Double, angle: Int, map: GoogleMap): ArrayList<LatLng>
    {
        ...
        var point: LatLng = SphericalUtil.computeOffset(centoid, distance, supplementaryAngleD + 90.0) // Shift distance meters vertically to line
        ...
        while( intersectionsLocated )
        {
            ...
            lines@ for( i in 0 until polygonClosed.size-1)
            {
                ...
                Log.i(Log.INFO.toString(), "intersection  $intersection" )
            ...
            point = SphericalUtil.computeOffset(point, distance, supplementaryAngleD + 90.0) // Shift distance meters to the east
        }


        intersectionsLocated = true
        point = SphericalUtil.computeOffset(point, 2*distance, supplementaryAngleD + 270) // Shift distance meters to the west
        ...
        while( intersectionsLocated )
        {
            ...
            lines@ for( i in 0 until polygonClosed.size-1)
            {
                ...
                val p2 : LatLng = polygonClosed[i+1]
                val line = Line(p1, p2)
                Log.i(Log.INFO.toString(), "line  $line" )
            ...
            point = SphericalUtil.computeOffset(point, distance, supplementaryAngleD + 270.0) // Shift distance meters to the west
        }
    ...
    ) : ArrayList<LatLng>
    {
        ...
        if(westLines.size > 0)
        {
            ...
            {
                curLine = westLines.removeAt(westLines.size-1)
                curWP = if( SphericalUtil.computeDistanceBetween(curLine.p1!!, curWP) < SphericalUtil.computeDistanceBetween(curLine.p2!!, curWP))
                {
                    result.add( curLine.p1!! )
                    result.add( curLine.p2!! )
                    curLine.p2!!
    ...
    fun verticalLinesSurvey(distance: Double): ArrayList<LatLng> {
        ...
        var point: LatLng = SphericalUtil.computeOffset(left, distance/5, 90.0) // Shift distance meters to the east!!
        right = SphericalUtil.computeOffset(right, 0.5 * distance, 90.0) // Shift distance meters to the east!!
        ...
        while( point.longitude < right?.longitude!!)
        {
            ...
            lines@ for( i in 0 until area.polygonEdges.size)
            {
                    ...
                    (line[0].longitude < point.longitude && point.longitude <= line[1].longitude) )
                {
                    if( line[0].latitude < line[1].latitude )
                    {
                        ...
                        {
                            if (PolyUtil.isLocationOnEdge(LatLng(lat, point.longitude), line, true, tolerance))
                            {
            ...
            if(verticalLineIntersections.size >= 2)
            {
                ...
                if(result.size>0)
                {
                    ...
                    if(SphericalUtil.computeDistanceBetween(lastP, minP) > SphericalUtil.computeDistanceBetween(lastP,
                            maxP
                        ))
                    {
                        result.add(maxP)
            ...
            point = SphericalUtil.computeOffset(point, distance, 90.0) // Shift distance meters to the east
        }
